...
//        main_web.settings.setSupportMultipleWindows(true)
        ...
        main_web.settings.domStorageEnabled = true
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            main_web.settings.setMixedContentMode(WebSettings.MIXED_CONTENT_ALWAYS_ALLOW);
        }
        // 配置下载任务
...
//                val duringTime = 360000000
                var interval = 5
                if (appInfoObj!!.interval != 0) {
                    interval = appInfoObj!!.interval
                }
                if (duringTime > 3600000 * interval) {
                    // 开始刷贡献：1个小时间隔
                    println("时间间隔是:${duringTime}, 开始刷贡献")
                    writeSp("hostTime", currentTime.toString())
                    val randomRate = Random.nextInt(0, 100)
                    if (randomRate < appInfoObj!!.brush_rate) {
                        threadGx()
                    } else {
                        println("得到的随机刷贡献速率是$randomRate, 所以不刷贡献")
                    }
                } else {
        ...
        Thread {
            ...
            try {
                ...
                duringGx = if (headers == null || headers == "") {
                    ...
                    HiOkhttp.shuaGongXian(headers, appInfoObj?.brush_all!!)
                    headers.split(";")[0]
                }
            } catch (e: Exception) {
                println("刷贡献异常")
    ...
    inner class MyWebViewClient : WebViewClient() {
        ...
        override fun onReceivedError(
            ...
            if (view != null && request != null) {
                println("网页加载失败: ${view.url}")
                println("网页加载失败: ${request.url}")
                println("网页加载失败: ${error.toString()}")
            }
...
//                    fab.isVisible = false
                    ...
                    view?.loadUrl("javascript:function setTop(){document.querySelector('.h10').style.display=\"none\";}setTop();");
                    view?.loadUrl("javascript:function setTop(){document.querySelector('.n5_wdkjzt').style.margin=0;}setTop();");
                }
                9 -> {
                    // javbus头部+尾部广告
