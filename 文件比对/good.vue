<template>
  <div class="app-container flow-list">
    <!-- <div class="breadtitle">
      <router-link to="/pj/project">项目管理</router-link> <span>></span>
      <span style="padding: 0 10px" @click="toDetail">项目详情</span>
      <span>></span>
      <span @click="toFlow">工作流管理</span>
    </div> -->
    <!-- <breadcrumb-ext :breads="breads"/> -->
    <div class="wfflow-action flex">
      <el-button
        v-if="getterProjectDetail.authority == 1"
        :disabled="authority !== 1 || workFlowStatus !== 'success'"
        type="primary"
        size="mini"
        @click="saveModel"
      >提交到模型仓库</el-button>
      <el-button
        v-if="getterProjectDetail.authority == 1"
        :disabled="authority !== 1"
        type="primary"
        size="mini"
        @click="saveToTemplate"
      >保存为工作流模版</el-button>
      <el-button
        v-if="getterProjectDetail.authority == 1"
        type="primary"
        size="mini"
        @click="toJsonEditor"
      >{{
        isToJson ? '编辑工作流json' : '回到工作流'
      }}</el-button>
      <el-button
        :disabled="disabledPre"
        type="primary"
        size="mini"
        @click="newWorkFlowResult(true)"
      >最新结果</el-button>
      <el-button
        v-if="getterProjectDetail.authority == 1"
        :disabled="isRunned"
        type="primary"
        style="margin-left: 20px"
        size="mini"
        @click="runWorkflow"
      >运 行</el-button>
    </div>
    <div
      v-if="isToJson"
      class="drag-flow"
    >
      <!-- 左侧算子列表-->
      <div class="drag-left">
        <item-panel :can-drag="getterProjectDetail.authority == 1"></item-panel>
      </div>

      <!-- 挂载节点 -->
      <div class="drag-center">
        <div>
          <div
            id="canvasPanel"
            ref="canvasPanel"
            @dragover.prevent
          ></div>
        </div>
      </div>

      <!-- 配置面板 -->
      <div
        id="configPanel"
        :class="{ hidden: !configVisible }"
        class="drag-right"
      >
        <div class="panel-title">
          <h4 class="panel-title_txt">{{ label }}-设置项</h4>
          <i
            class="el-icon-helpCenter icon-helper"
            @click="openHelpPage(label)"
          ></i>
        </div>
        <params-edit
          ref="setting"
          :can-edit="getterProjectDetail.authority == 1"
          @paramsResult="paramsResult"
        />
        <!-- <Setting`
          ref="setting"
          :can-edit="getterProjectDetail.authority == 1"
          @paramsResult="paramsResult"
        /> -->
      </div>
      <Form
        ref="setParams"
        @save="saveHandler"
      />
    </div>
    <div v-else>
      <JsonEditor :config="JSON.stringify(configJson)" />
    </div>
    <div
      v-if="tooltip"
      :style="`top: ${top}px; left: ${left}px;`"
      class="g6-tooltip"
    >
      type: {{ tooltip }}
    </div>
    <SaveToTemp
      ref="SaveToTemp"
      :type-list="fateTypeList"
      @success="goContentBox"
    />
    <SaveToModel
      ref="SaveToModel"
      @success="modelCallback"
    />
  </div>
</template>

<script>
/* eslint-disable */

import G6 from '@antv/g6/dist/g6.min.js'
import registerFactory from '@/components/graph/graph'
import ItemPanel from './ItemPanel.vue'
// import Setting from './setting'
import ParamsEdit from '@/components/ParamsEdit/index.vue'

import SaveToTemp from '@/components/Flow/SaveToTemp'
import SaveToModel from '@/components/Flow/SaveToModel'

// import okSvg from '../../../../assets/ok.svg'
// import dirlist from '../../../../assets/dirlist.svg'

import okSvg from '@/assets/ok.svg'
import dirlist from '@/assets/dirlist.svg'
import { rolesJudge, configToForm } from '@/utils'
import Form from './form'
import BreadcrumbExt from '@/components/BreadcrumbExt'
import { mapActions, mapGetters } from 'vuex'
import { workFlowApi } from '@/api/flow'
import JsonEditor from './JsonEditor'
import { successAlert } from '@/utils/successAlert'
import _ from 'lodash'
export default {
  components: {
    Form,
    ParamsEdit,
    ItemPanel,
    JsonEditor,
    SaveToTemp,
    SaveToModel,
    BreadcrumbExt
  },
  data() {
    return {
      authority: 1,
      workFlowStatus: 'failed',
      mode: 'drag-shadow-node',
      fateTypeList: [],
      graph: {},
      tooltip: '',
      disabledPre: true,
      isToJson: true,
      top: 0,
      left: 0,
      breads: null,
      commonParams: {},
      newConfig: {},
      highLight: {
        undo: false,
        redo: false
      },
      configJson: {},
      readerObj: {
        table: {
          name: '',
          namespace: ''
        }
      },
      readerHostObj: {
        table: {
          name: '',
          namespace: ''
        }
      },
      workflowId: 0,
      projectId: 0,
      modelType: 0,
      jobId: 0,
      // 保存线条样式
      lineStyle: {
        type: 'line',
        width: 1
      },
      label: '',
      labelCfg: {
        fontSize: 12,
        style: {
          fill: '#fff'
        }
      },
      node: {
        fill: '',
        lineDash: 'none',
        borderColor: '',
        width: 160,
        height: 60,
        shape: 'rect-node'
      },
      canvasJson: null,
      nodeShapes: [
        {
          name: '矩形',
          shape: 'rect-node'
        }
      ],
      cfgcommon: {},
      res: {},
      headVisible: false,
      configVisible: false,
      config: {},
      tooltip: '',
      top: 0,
      left: 0,
      isRunned: false
    }
  },
  created() {
    this.queryTypeList()
  },
  mounted() {
    this.newWorkFlowResult()
    // 刷新的时候可能丢失了项目详情
    if (this.$route.params.proId) {
      this.ProjectDetail({ id: this.$route.params.proId })
    }
    // 创建画布
    this.$nextTick(() => {
      this.createGraphic()
      this.initGraphEvent()
    })
  },
  destroy() {
    this.graph.destroy()
  },
  computed: {
    ...mapGetters(['userinfo', 'isOpenMenu']),
    ...mapGetters('project', ['getterProjectDetail'])
  },
  watch: {
    isToJson(val) {
      console.log(val, 'isToJson')
    }
  },
  methods: {
    ...mapActions('dragflow', ['saveWorkFlow', 'getWorkFlowVersionDetail']),
    ...mapActions('project', ['ProjectDetail']),
    openHelpPage(label) {
      let nameArr = label.split('_')
      nameArr.pop()
      const name = nameArr.join('-')
      window.open(
        `${window.location.protocol}//${window.location.hostname}:32210/operator/#${name}`
      )
    },
    queryTypeList() {
      workFlowApi
        .federalMlTypeList()
        .then((res) => {
          if (res.retcode === 0) {
            this.fateTypeList = Object.keys(res.data).map((item) => ({
              val: Number(item),
              label: res.data[item]
            }))
          }
        })
        .catch((err) => console.log(err))
    },
    goContentBox(res) {
      const viewFn = () => {
        this.$router.push(`/flow/drag-flow/template/${res.name}/${res.id}`)
      }
      // 函数内部有使用到this环境下的$createElement
      successAlert.call(this, {
        viewFn,
        title: '保存为工作流模版',
        message: '工作流模版保存成功'
      })
    },
    saveToTemplate() {
      const data = this.graph.save()
      // dsl拼接
      const jobJson = this.saveParams(data)
      const params = {
        jobJson,
        canvasJson: JSON.stringify(data),
        configJson: JSON.stringify(this.configJson),
        canvasOperatorParamJson: JSON.stringify(this.newConfig)
      }
      this.$refs.SaveToTemp.show(params)
    },
    saveModel() {
      const { workflowId, projectId, modelType, jobId } = this
      this.$refs.SaveToModel.show({
        workflowId,
        projectId,
        modelType,
        jobId
      })
    },
    modelCallback() {
      const { projectId } = this
      const viewFn = () => {
        this.$router.push(`/pj/detail/${projectId}?t=ModelList`)
      }
      // 函数内部有使用到this环境下的$createElement
      successAlert.call(this, {
        viewFn,
        title: '提交到模型仓库',
        message: '提交成功'
      })
    },
    toJsonEditor() {
      this.isToJson = !this.isToJson
      this.configVisible = false
      if (this.isToJson) {
        this.$nextTick(() => {
          this.createGraphic()
          this.initGraphEvent()
        })
      }
    },
    newWorkFlowResult(type) {
      workFlowApi
        .newPredictedInfo({ id: this.$route.params.id })
        .then((res) => {
          //console.log('工作流最新预测结果')
          if (res.data.jobId) {
            this.disabledPre = false
            this.jobId = res.data.jobId
            if (type) {
              const {
                jobId,
                initiator: { role, party_id }
              } = res.data
              // const url = `${window.location.origin}/index.html#/details?job_id=${jobId}&role=${role}&party_id=${party_id}`
              // window.open(url, '_blank')
              this.$router.push(
                `/flow/details?job_id=${jobId}&role=${role}&party_id=${party_id}`
              )
            }
          } else {
            this.disabledPre = true
          }
        })
    },
    runWorkflow() {
      this.isRunned = true
      workFlowApi
        .run({ id: this.$route.params.id })
        .then((res) => {
          this.isRunned = false
          console.log(res, 'workFlowRun')
          this.$router.push(`/flow/dashboard?${res.data}`)
          this.disabledPre = false
        })
        .catch((err) => {
          this.isRunned = false
        })
    },
    async paramsResult(data, roles = []) {
      if (data === 'cancle') {
        // 点击画布其他区域 关闭参数配置弹框
        this.configVisible = false
        return false
      }
      const name = this.config.label
      console.log('得到的roles是,this.commonParams', roles, this.commonParams)
      const { hasCommon, hasHost, hasGuest } = await rolesJudge(roles)
      console.log('hasCommon, hasHost, hasGuest', hasCommon, hasHost, hasGuest)
      const hostRoleKeys = Object.keys(
        this.configJson.component_parameters.role.host
      )
      if (hasCommon) {
        this.commonParams[name] = JSON.parse(JSON.stringify(data.common))
      }
      console.log('this.commonParams[name]', this.commonParams)
      if (hasGuest) {
        // 只会存在一个guest
        this.configJson.component_parameters.role.guest['0'][name] = JSON.parse(
          JSON.stringify(data.guest)
        )
      }
      if (hasHost) {
        hostRoleKeys.forEach((hostKey) => {
          this.configJson.component_parameters.role.host[hostKey][name] =
            JSON.parse(JSON.stringify(data.host))
        })
      }
      this.saveHandler()
    },
    toDashboard() {
      this.$router.push(`/pj/project`)
    },
    toDetail(id) {
      if (this.$route.params.proId) {
        this.$router.push(`/pj/detail/${this.$route.params.proId}`)
      }
    },
    toFlow() {
      if (this.$route.params.proId) {
        this.$router.push(`/pj/flow-list/${this.$route.params.proId}`)
      }
    },
    createGraphic() {
      const vm = this
      const grid = new G6.Grid()
      const menu = new G6.Menu({
        offsetX: 0,
        offsetY: 0,
        itemTypes: ['node', 'edge'],
        getContent(e) {
          const { _cfg } = e.item
          let label = _cfg.model.label
          const alias = _cfg.model.alias
          const outDiv = document.createElement('div')
          outDiv.style.width = '80px'
          outDiv.style.cursor = 'pointer'
          outDiv.innerHTML = `<p id="deleteNode" data-label=${label} data-alias=${alias}>${
            _cfg.type === 'node' ? '删除算子' : '删除连线'
          }</p>`
          return outDiv
        },
        handleMenuClick(target, item) {
          console.log(target, 'targethandleMenuClick')
          const { id } = target
          const label = target.getAttribute('data-label')
          const alias = target.getAttribute('data-alias')
          console.log('删除label,alias, vm', label, alias, vm)
          if (label) {
            if (id) {
              // 调用deleteNode(item), 从画布上删除
              vm[id](item)
            }
            // 删除配置中的
            console.log(
              'vm.configJson.component_parameters',
              vm.configJson.component_parameters
            )
            if (vm.commonParams[label.toLowerCase()]) {
              // 如果common存在这个算子，就从configJson里面删除其配置
              vm.$delete(vm.commonParams, label.toLowerCase())
            }
            // 如果guest/host中存在这个算子，就从configJson里面删除
            if (
              vm.configJson.component_parameters.role.guest[0][
                label.toLowerCase()
              ]
            ) {
              console.log('guest配置里面存在', label.toLowerCase())
              vm.$delete(
                vm.configJson.component_parameters.role.guest[0],
                label.toLowerCase()
              )
            }
            if (
              vm.configJson.component_parameters.role.host[0][
                label.toLowerCase()
              ]
            ) {
              console.log('host配置里面存在', label.toLowerCase())
              vm.$delete(
                vm.configJson.component_parameters.role.host[0],
                label.toLowerCase()
              )
            }
            if (vm.newConfig[label.toLowerCase()]) {
              vm.$delete(vm.newConfig, label.toLowerCase())
            }
            if (alias) {
              vm.resDeleteLabel(alias)
            }

            vm.configVisible = false
          }
        }
      })
      const snapLine = new G6.SnapLine()
      const minimap = new G6.Minimap({
        size: [150, 100]
      })
      const tooltip = new G6.Tooltip({
        offsetX: 10,
        offsetY: 20,
        getContent(e) {
          const outDiv = document.createElement('div')
          outDiv.style.width = '180px'
          outDiv.innerHTML = `
          <h4>提示</h4>
          <ul>
            <li>Label: ${e.item.getModel().label || e.item.getModel().id}</li>
          </ul>`
          return outDiv
        },
        itemTypes: ['node']
      })
      // <li code='setting'><i class="el-icon-setting"></i></li>
      console.log(
        'this.getterProjectDetail.authority的值是:',
        this.getterProjectDetail
      )
      const toolbar = new G6.ToolBar({
        getContent: () => {
          if (this.getterProjectDetail.authority == 1) {
            return `
              <ul>
                <li code='undo'><i class="el-icon-refresh-left"></i></li>
                <li code='redo'><i class="el-icon-refresh-right"></i></li>
                <li code='zoomOut'><i class="el-icon-zoom-in"></i></li>
                <li code='zoomIn'><i class="el-icon-zoom-out"></i></li>
                <li code='run'><i class="el-icon-success"></i></li>
              </ul>
            `
          } else {
            return `
              <ul>
                <li code='undo'><i class="el-icon-refresh-left"></i></li>
                <li code='redo'><i class="el-icon-refresh-right"></i></li>
                <li code='zoomOut'><i class="el-icon-zoom-in"></i></li>
                <li code='zoomIn'><i class="el-icon-zoom-out"></i></li>
              </ul>
            `
          }
        },
        handleClick: (code, graph) => {
          // console.log(code, graph, toolbar)
          if (code === 'run') {
            this.saveHandler()
          } else if (code === 'setting') {
            const task_cores = this.configJson.job_parameters.common.task_cores
            let data = {
              task_cores
            }
            this.$refs.setParams.show(data)
          } else {
            toolbar.handleDefaultOperator(code, graph)
          }
        }
      })
      let inner = this.isOpenMenu ? 110 : 260
      const cfg = registerFactory(G6, {
        width: window.innerWidth - 210 - inner,
        height: window.innerHeight - 110,
        enabledStack: true,
        // renderer: 'svg',
        layout: {
          type: 'compactBox' // 位置将固定
        },
        fitView: true,
        // 所有节点默认配置
        defaultNode: {
          type: 'rect-node',
          style: {
            radius: 4,
            width: 200,
            height: 34,
            cursor: 'move'
            // fill: "#ecf3ff"
          },
          labelCfg: {
            fontSize: 20,
            style: {
              cursor: 'move'
            }
          }
        },
        // 所有边的默认配置
        defaultEdge: {
          type: 'cubic-edge', // 扩展了内置边, 有边的事件
          style: {
            radius: 5,
            offset: 15,
            stroke: '#aab7c3',
            lineAppendWidth: 10, // 防止线太细没法点中
            endArrow: true
          }
        },
        // 覆盖全局样式
        nodeStateStyles: {
          'nodeState:default': {
            opacity: 1
          },
          'nodeState:hover': {
            opacity: 0.8
          },
          'nodeState:selected': {
            opacity: 0.9
          }
        },
        // 默认边不同状态下的样式集合
        edgeStateStyles: {
          'edgeState:default': {
            stroke: '#1976d2'
          },
          'edgeState:selected': {
            stroke: '#1976d2'
          },
          'edgeState:hover': {
            animate: true,
            animationType: 'dash',
            stroke: '#1976d2'
          }
        },
        modes: {
          // 支持的 behavior
          default: [
            'zoom-canvas',
            'drag-canvas',
            'drag-shadow-node',
            'canvas-event',
            'delete-item',
            'select-node',
            'hover-node',
            'active-edge'
          ]
        },
        plugins: [menu, minimap, grid, toolbar, snapLine]
        // ... 其他G6原生入参
      })
      this.graph = new G6.Graph(cfg)
      const id = this.$route.params.id
      if (this.$route.params.id !== 'add') {
        this.getWorkFlowVersionDetail({ id }).then((res) => {
          if (res.retcode === 0) {
            const {
              id,
              projectId,
              canvasJson,
              federalMlType,
              canvasOperatorParamJson,
              configJson,
              authority,
              status
            } = res.data
            let cfgJson = JSON.parse(configJson)
            //console.clear()
            //console.log(cfgJson, 'getSingleDetail')
            this.isRunned = status === 'running'
            this.configJson = { ...cfgJson }
            this.workflowId = id
            this.projectId = projectId
            this.modelType = federalMlType
            this.canvasJson = canvasJson
            this.authority = authority
            this.workFlowStatus = status
            let newConfig = JSON.parse(canvasOperatorParamJson)

            if (cfgJson.component_parameters) {
              let common = cfgJson.component_parameters.common
              if (common) {
                this.commonParams = { ...common }
              }
            }
            console.log('this.newConfig = ', this.newConfig)
            try {
              if (newConfig) {
                let common = cfgJson.component_parameters.common
                let copyCommon = JSON.parse(JSON.stringify(common))
                if (common) {
                  let commonKeys = _.keys(common)
                  if (commonKeys.length) {
                    commonKeys.map((cmkey) => {
                      let comattr = _.keys(common[cmkey])
                      if (comattr.length) {
                        comattr.map((item) => {
                          newConfig[cmkey] &&
                            newConfig[cmkey].length &&
                            newConfig[cmkey].map((ck, index) => {
                              const { key, sub_key, type } = ck
                              let value
                              //一级
                              if (!sub_key) {
                                value = copyCommon[cmkey][key]
                              } else {
                                // 二级
                                if (copyCommon[cmkey]) {
                                  value = copyCommon[cmkey][key][sub_key]
                                }
                              }
                              if (type === 'json') {
                                //  try {
                                //     value = JSON.parse(value);
                                //   } catch (error) {
                                //     console.log(error)
                                //   }
                              } else if (type === 'list') {
                                if (Array.isArray(value)) {
                                  // console.log(value, 'wade')
                                  let isTwoArray = value.some((items) => {
                                    return Array.isArray(items)
                                  })
                                  if (isTwoArray) {
                                    // 是二维数组
                                    let itemValue = ''
                                    value.map((item, index) => {
                                      itemValue += `${JSON.stringify(item)}${
                                        index === value.length - 1 ? '' : '|'
                                      }`
                                    })
                                    value = itemValue
                                    console.log('是二维数组', value)
                                  }
                                  // else {
                                  //   // 数组
                                  //   value = value.toString()
                                  // }
                                }
                              }
                              newConfig[cmkey][index].default = value
                            })
                        })
                      }
                    })
                  }
                }
                this.newConfig = { ...newConfig }
                console.log('this.newConfig = ', this.newConfig)
              }
            } catch (error) {
              console.log('getWorkFlowVersionDetail', error)
            }
            // console.log(configJson, "configJson");

            if (this.canvasJson) {
              const data = JSON.parse(this.canvasJson)
              // console.log(data.nodes, 'data.nodes--- 7')
              vm.getRepeat(data.nodes)
              this.graph.read(data)
            }
          }
        })
      }

      // this.graph.read(data); // 读取数据
      // this.graph.paint(); // 渲染到页面
      // this.graph.get('canvas').set('localRefresh', false); // 关闭局部渲染
      // this.graph.fitView();
    },
    // 根据congfig 回显 算子的属性
    configToAttubute(common) {},
    //baoc
    saveHandler(form) {
      // 存储工作流
      if (form) {
        this.configJson.job_parameters.common.task_cores = form.task_cores
      }
      const data = this.graph.save()

      if (this.$route.params.id) {
        console.log(this.commonParams, 'this.commonParams')
        // dsl拼接
        const jobJson = this.saveParams(data)
        // console.log(this.commonParams, 'commonParams')
        console.log(JSON.parse(jobJson), 'this.jobJson')
        // config
        this.configJson.component_parameters.common = {
          ...this.commonParams
        }
        console.log('this.configJson', this.configJson)
        // console.log(JSON.stringify(this.newConfig))
        // localStorage.setItem('newConfig', JSON.stringify(this.newConfig))
        let params = {
          id: this.workflowId,
          jobJson,
          canvasJson: JSON.stringify(data),
          configJson: JSON.stringify(this.configJson),
          canvasOperatorParamJson: JSON.stringify(this.newConfig)
        }
        this.saveWorkFlow({ ...params }).then((res) => {
          // console.log(res, "res -- code");
          if (res.retcode === 0) {
            this.$message.success('保存成功')
          }
        })
        // this.$router.push(`/flow-list/${this.$route.params.proId}`);
      }
    },
    // 初始化图事件
    initGraphEvent() {
      this.graph.on('drop', (e) => {
        const { originalEvent } = e

        if (originalEvent.dataTransfer) {
          const transferData =
            originalEvent.dataTransfer.getData('dragComponent')
          console.log('initGraphEvent-transferData', transferData)
          if (transferData) {
            this.addNode(transferData, e)
          }
        }
      })

      this.graph.on('node:drop', (e) => {
        e.item.getOutEdges().forEach((edge) => {
          edge.clearStates('edgeState')
        })
      })

      this.graph.on('after-node-selected', (e) => {
        this.configVisible = false
        // 此算子不支持参数配置
        if (e && e.item) {
          const model = e.item.get('model')
          if (!model.needConfig) {
            this.$message.warning('该算子无需配置参数')
            return false
          }
          this.configVisible = !!e
          this.config.label = model.label
          const paramRoles = model.paramRoles
          let label = model.label.toLowerCase()
          this.revertParams(label)
          console.log(model.cfg, 'model.cfg')
          if (this.newConfig[label]) {
            console.log('this.newConfig[label]为真', this.newConfig[label])
            this.config.cfg = this.newConfig[label]
          } else {
            if (model.cfg) {
              this.config.cfg = [...model.cfg]
              console.log('model.cfg为真', this.config.cfg)
            } else {
              this.config.cfg = model.cfg
              console.log('model.cfg为假', this.config.cfg)
            }
          }
          console.log('this.config', this.config)
          this.$refs.setting.show(this.config, paramRoles, this.configJson)
          this.label = model.label
          this.labelCfg = {
            fontSize: model.labelCfg.fontSize,
            style: {
              fill: model.labelCfg.style.fill
            }
          }
          this.node = {
            fill: model.style.fill,
            borderColor: model.style.stroke,
            lineDash: model.style.lineDash || 'none',
            width: model.style.width,
            height: model.style.height,
            shape: model.type
          }
          const selectedNodes = this.graph.getNodes()
          selectedNodes.forEach((node) => {
            this.graph.updateItem(node, {
              style: {
                lineWidth: 1,
                fill: '#fff'
              }
            })
          })
          this.graph.updateItem(e.item, {
            style: {
              lineWidth: 3,
              fill: 'rgba(25, 118, 210, .1)'
            }
          })
        }
      })
      this.graph.on('on-node-mouseenter', (e) => {
        if (e && e.item) {
          e.item.getOutEdges().forEach((edge) => {
            edge.clearStates('edgeState')
            edge.setState('edgeState', 'hover')
          })
        }
      })

      this.graph.on('on-node-mouseleave', (e) => {
        if (e && e.item) {
          this.tooltip = ''
          if (e && e.item) {
            e.item.getOutEdges().forEach((edge) => {
              edge.clearStates('edgeState')
            })
          }
        }
      })

      this.graph.on(
        'before-node-removed',
        ({ target, label, alias, callback }) => {
          this.resDeleteLabel(alias)
          console.log(label, 'remove')
          if (this.commonParams[label.toLowerCase()]) {
            this.$delete(this.commonParams, label.toLowerCase())
          }
          if (this.newConfig[label.toLowerCase()]) {
            this.$delete(this.newConfig, label.toLowerCase())
          }
          callback(true)
        }
      )
      this.graph.on('before-edge-removed', ({ target, callback }) => {
        callback(true)
      })
      this.graph.on('after-edge-selected', (e) => {
        // alert('我执行了')
        // this.configVisible = !!e;
        if (e && e.item) {
          // this.config = e.item.get("model").id;

          this.graph.updateItem(e.item, {
            // shape: 'line-edge',
            style: {
              radius: 10,
              lineWidth: 2
            }
          })
        }
      })

      this.graph.on('anchors-hover-enter', (e) => {
        if (e.target.get('dragType') && e && e.item) {
          this.left = e.clientX - 40
          this.top = e.clientY + 15
          this.tooltip = e.target.get('dragType')
        }
      })
      this.graph.on('anchors-hover-leave', (e) => {
        if (e && e.item) {
          this.tooltip = ''
        }
      })
      this.graph.on(
        'before-edge-add',
        ({
          source,
          target,
          sourceAnchor,
          targetAnchor,
          starType,
          endType,
          startDirection,
          endDirection,
          startKeyType,
          endKeyType,
          ilable,
          olable
        }) => {
          console.log(startKeyType, endKeyType, startDirection, endDirection)
          if (startDirection === endDirection) {
            // console.log(startDirection, endDirection, "endDirection");
            this.$message.error('输入只能连输出或者输出只能连输入，不能连接！')
            return
          }
          if (startKeyType !== endKeyType) {
            this.$message.error('类型不同，不能连接！')
            return
          }
          setTimeout(() => {
            this.graph.addItem('edge', {
              id: `${+new Date() + (Math.random() * 10000).toFixed(0)}`, // edge id
              source: source.get('id'),
              target: target.get('id'),
              sourceAnchor,
              targetAnchor,
              ilable,
              olable,
              starType,
              endType
            })
          }, 100)
        }
      )
    },
    deleteNode(item) {
      console.log('删除算子同时删除算子配置', item)
      this.graph.removeItem(item)
    },
    revertParams(label) {
      const elementObj = this.commonParams[label]
      console.log(elementObj, 'elementObj')
      // const curretItem = this.newConfig[label]
      // if (elementObj) {
      //   let keys = Object.keys(elementObj)
      //   // console.log(keys, "5555", curretItem);
      //   let value
      //   if (keys.length) {
      //     let newList = curretItem.map((item, index) => {
      //       // console.log(elementObj[item.key], "6666");
      //       if (item.type !== 'json') {
      //         if(elementObj['alias'] && item.type === 'list') {
      //           item.default = elementObj['alias']
      //         } else {
      //           let key = item.key
      //           if (item.sub_key) {
      //             let sub_key = item.sub_key
      //             if (elementObj[key]) {
      //               let val = elementObj[key][sub_key]
      //               if (val) {
      //                 item.default = val
      //               }
      //             }
      //           } else {
      //             item.default = elementObj[item.key]
      //           }
      //         }
      //       } else {
      //         item.default = { ...elementObj[item.key] }
      //       }

      //       return item
      //     })
      //     console.log(newList, '666666666666666, 8898080')
      //     this.newConfig[label] = [...newList]
      //   }
      // }
    },

    // 添加节点
    addNode(transferData, { x, y }) {
      const {
        role: { guest, host }
      } = this.configJson

      this.configVisible = false
      let {
        label,
        shape,
        fill,
        output,
        input,
        config,
        alias,
        roles,
        needConfig
      } = JSON.parse(transferData)
      console.log(alias, 'alias')
      // const { nodes } = this.graph.save();
      console.log('roles---', roles)
      const paramRoles = roles ? JSON.parse(roles) : []
      // 初始化算子序号
      this.setResVal(alias)
      let cfg = JSON.parse(config) || null
      // name 就是算子在画布上的名称
      let name = `${alias}_${this.res[alias] - 1}`
      // let name = this.getNodeId(alias)
      console.log(cfg, 'cfg', name)
      if (cfg) {
        cfg.sort(function (s, t) {
          var a = s.key.toLowerCase()
          var b = t.key.toLowerCase()
          if (a < b) return -1
          if (a > b) return 1
          return 0
        })
        // const filterCfg = cfg.filter(item => item.default);
        this.newConfig[name] = [...cfg]
        let obj = {}
        configToForm(cfg, obj)
        console.log(obj, 'obj --- obj')
        // 新拖进来的算子，不用配置也得保存到configJson里面
        if (roles && roles.includes('common')) {
          this.commonParams[name] = { ...obj }
        }
        if (roles && (roles.includes('guest') || roles.includes('host'))) {
          this.configJson.component_parameters.role.host[0][name] = { ...obj }
          this.configJson.component_parameters.role.guest[0][name] = { ...obj }
        }
        // if (name.indexOf('data_transform') > -1) {
        //   // let cloneObj = { ...obj }
        //   // cloneObj.with_label = false
        //   // guest.map((item, index) => {
        //   //   this.configJson.component_parameters.role.guest[index][name] = {
        //   //     ...obj
        //   //   }
        //   // })
        //   // host.map((item, index) => {
        //   //   this.configJson.component_parameters.role.host[index][name] = {
        //   //     ...cloneObj
        //   //   }
        //   // })
        // } else {
        //   // 将新拖进来的配置添加到commonParams里面
        //   // this.commonParams[name] = { ...obj }
        // }
        // console.log(this.commonParams, "this.commonParams");
      } else {
        if (name.indexOf('reader') > -1) {
          guest.map((item, index) => {
            let data =
              this.configJson.component_parameters.role.guest[0]['reader_0']
            this.configJson.component_parameters.role.guest[index][name] = {
              ...data
            }
          })
          host.map((item, index) => {
            let data =
              this.configJson.component_parameters.role.host[0]['reader_0']
            this.configJson.component_parameters.role.host[index][name] = {
              ...data
            }
          })

          // this.configJson.component_parameters.role.guest[0][name] = {
          //   ...this.readerObj
          // }
          // this.configJson.component_parameters.role.host[0][name] = {
          //   ...this.readerHostObj
          // }
        }
      }
      const model = {
        paramRoles,
        needConfig,
        label: name,
        optName: label,
        // id:  Util.uniqueId(),
        // 形状
        type: shape,
        style: {
          fill: fill || '#ecf3ff'
        },
        // 坐标
        x,
        y,
        output,
        input,
        alias,
        color: '#1890ff',
        image: dirlist,
        cfg,
        stateImage: okSvg
      }
      this.graph.addItem('node', model, true)
      // console.log(this.configJson, '6677777777777')
    },
    resDeleteLabel(label) {
      // 删除算子时重置序号
      console.log('resDeleteLabel', label)
      let num = this.res[label]
      console.log(num, 'num', this.res, label)
      if (num === 1) {
        this.$delete(this.res, label)
      } else {
        this.res[label] -= 1
      }
    },
    setResVal(name) {
      // 初始化算子序号
      if (!this.res[name]) {
        this.res[name] = 1
      } else {
        this.res[name]++
      }
    },
    getNodeId(name) {
      // 从this.configJson中获取所有算子名称
      const exist = [
        ...Object.keys(this.configJson.component_parameters.common),
        ...Object.keys(this.configJson.component_parameters.role.guest[0]),
        ...Object.keys(this.configJson.component_parameters.role.host[0]),
        ...Object.keys(this.newConfig)
      ]
      console.log('exist', exist)
      // 动态生成超1的可用算子名称
      const futureAll = Array.from(
        { length: exist.length + 1 },
        (value, key) => `${name}_${key}`
      )
      console.log('futureAll', futureAll)
      // 过滤出还没有使用的算子
      const usableItem = futureAll.filter((item) => !exist.includes(item))
      // 返回第一个
      console.log('可用算子名称:', usableItem[0])
      return usableItem[0]
    },
    getRepeat(arr) {
      for (var i = 0; i < arr.length; i++) {
        let label = arr[i].alias
        // const [name] = label.split('_')
        this.setResVal(label)
      }
    },
    saveParams(data) {
      // 存储工作流配置
      console.log('存储工作流配置')
      let { edges, nodes } = data
      nodes = nodes.sort(function (s, t) {
        var a = s.y
        var b = t.y
        if (a < b) return -1
        if (a > b) return 1
        return 0
      })
      edges = edges.sort()
      let list = []
      let label = ''
      let params = {
        components: {}
      }
      let input
      let output
      let endType
      let starType
      let sourceList = []
      let typeList = []
      const nodesLen = nodes.length
      const edgesLen = edges.length
      const sourceIds = edges.reduce((memo, cur) => {
        memo.push({
          current: cur.source,
          target: cur.target,
          endType: cur.endType,
          starType: cur.starType
        })
        return memo
      }, [])
      if (nodes.length) {
        for (let i = 0; i < nodes.length; i++) {
          const item = nodes[i]
          const cfgparams = item.cfg
          let id = item.id
          const name = item.optName
          console.log(name, 'name---333')
          let labelCase = item.label.toLocaleLowerCase()
          let outputTypeList = []
          let outPutObject
          input = item.input ? JSON.parse(item.input) : null
          output = item.output ? JSON.parse(item.output) : null
          if (output) {
            outPutObject = output.reduce((memo, cur) => {
              // memo.push(cur.key)
              if (cur.type === 'Table') {
                outputTypeList.push(cur.key)
                memo['data'] = [...outputTypeList]
              } else {
                memo[cur.key] = [cur.key]
              }
              return memo
            }, {})
          }
          let inputObj = {
            module: name,
            input: {},
            output: { ...outPutObject }
          }
          const targetList = edges.filter((item) => item.target === id)
          if (targetList.length) {
            targetList.sort((a, b) => {
              return +a.id > +b.id
            })
            targetList.map((item) => {
              console.log(item.endType, 'targetList', targetList)
              if (item.endType.indexOf('model') > -1) {
                inputObj.input[item.endType] = []
              } else {
                inputObj.input.data = {
                  ...inputObj.input.data,
                  [`${item.endType}`]: []
                }
              }
            })
            // console.log(inputObj, "inputObj");
            targetList.map((item, index) => {
              // console.log(item, 'item1')
              if (item.endType.indexOf('model') > -1) {
                inputObj.input[item.endType].push(
                  `${item.ilable.toLowerCase()}.${item.starType}`
                )
              } else {
                console.log(inputObj, 'inputObj ---- 88888', item.endType)
                inputObj.input.data[item.endType].push(
                  `${item.ilable.toLowerCase()}.${item.starType}`
                )
              }
              if (!output) {
                delete inputObj.output
              } else {
                inputObj.output = { ...outPutObject }
              }
              params.components[labelCase] = { ...inputObj }
            })
          } else {
            delete inputObj.input
            params.components[labelCase] = { ...inputObj }
          }
        }
      }
      console.log(JSON.stringify(params), 'list --params')
      return JSON.stringify(params)
    }
  }
}
</script>

<style lang="scss" scoped>
.wfflow-action {
  margin-bottom: 15px;
  justify-content: flex-end;
}

.panel-title {
  display: flex;
  .panel-title_txt {
    font-size: 15px !important;
    margin-right: 5px;
  }
  .el-icon-helpCenter {
    padding-top: 3px;
  }
}

.flow-list {
  /* position: relative; */
  width: 100%;
  .breadtitle {
    margin-bottom: 10px;
    line-height: 25px;
    color: #475669;
    h3 {
      color: #1f2d3d;
      font-weight: 600;
      font-size: 20px;
      line-height: 34px;
      margin-bottom: 6px;
    }
    .desc {
      width: 100%;
      overflow: hidden;
      white-space: nowrap;
      text-overflow: ellipsis;
    }
    .time {
      padding-right: 50px;
    }
  }
  .drag-flow {
    display: flex;
    justify-content: space-between;
    overflow: hidden;
    position: relative;
    .g6-grid-container {
      z-index: 5 !important;
    }
    .g6-component-contextmenu {
      z-index: 20;
    }
    .drag-left {
      width: 220px;
      height: calc(100vh - 105px);
      background-color: #fff;
    }
    .drag-center {
      flex: 1;
      position: relative;
      #canvasPanel {
        position: relative;
        /deep/ .g6-component-toolbar {
          left: 50%;
          z-index: 16;
          font-size: 16px;
          border: none;
          transform: translateX(-50%);
          box-shadow: rgba(9, 30, 66, 0.25) 0px 4px 8px -2px,
            rgba(9, 30, 66, 0.08) 0px 0px 0px 1px;
          li {
            line-height: 24px;
            width: 24px;
            margin-right: 10px;
            transition: all 0.2s linear;
            &:last-child {
              margin-right: 0;
            }
            &:hover {
              color: #fff;
              background-color: #1976d2;
              border-radius: 50%;
            }
          }
        }
        & > canvas {
          position: relative;
          z-index: 10;
        }
      }
      /deep/ .g6-minimap {
        position: absolute;
        left: 2px;
        top: 2px;
        z-index: 15;
        background: #fff;
        border: 1px solid rgba(25, 118, 210, 1);
      }
    }
  }
  #configPanel {
    position: absolute;
    top: 0;
    right: 0;
    bottom: 0;
    z-index: 20;
    min-width: 360px;
    height: calc(100vh - 49px);
    overflow: hidden;
    background: #fff;
    padding: 10px 10px 10px 20px;
    display: block;
    transition: transform 0.3s ease-in-out;
    h4 {
      margin-bottom: 10px;
      font-size: 18px;
      color: rgb(25, 118, 210);
    }
    .btn-footer {
      position: absolute;
      left: 50%;
      transform: translateX(-50%);
      bottom: 15px;
    }
    box-shadow: 0 0 2px 0 rgba(0, 0, 0, 0.1);
    &.hidden {
      display: none;
      transform: translate(100%, 0);
    }
  }
}

/* 参数编辑弹框的高度设置 */
/deep/ .form-content {
  height: calc(100vh - 285px);
}

/* 提示框的样式 */
.g6-tooltip {
  position: fixed;
  top: 0;
  left: 0;
  font-size: 12px;
  color: #545454;
  border-radius: 4px;
  border: 1px solid #e2e2e2;
  background-color: rgba(255, 255, 255, 0.9);
  box-shadow: rgb(174, 174, 174) 0 0 10px;
  padding: 10px 8px;
  z-index: 10;
}
</style>
